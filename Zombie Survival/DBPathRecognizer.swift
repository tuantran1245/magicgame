...
open class DBPathRecognizer {
    ...
    fileprivate func directions() -> [Int] {
        ...
        var result: [Int] = []
        let sliceAngle:Double = Double.pi * 2.0 / Double(sliceCount)
        ...
        for (index, _) in dpoints.enumerated() {
            if index < dpoints.count - 1 {
                ...
                if (angle < sliceAngle / 2.0 || angle > Double.pi * 2.0 - sliceAngle){
                    result.append(0)
                } else {
    ...
    fileprivate func costLeven(_ a:[Int],_ b:[Int]) -> Int {
        ...
        //for x in 1...a.count
        for x in stride(from: 1, through: a.count, by: 1)
        {
           // for y in 1..<b.count
            for y in stride(from: 1, to: b.count, by: 1)
            {
        ...
        for index in stride(from: 1, to: b.count, by: 1) {
        ...
        {
            //for y in 1..<b.count
            for y in stride(from: 1, through: b.count - 1, by: 1)
            {
                cost = td[x, y];
                pa = tw[x - 1, y] + cost;
                pb = tw[x, y - 1] + cost;
...
public struct PathInfos {
    ...
    var width:Int {
        get {
            return Int (boundingBox.right) - Int (boundingBox.left)
        }
    }
    
    var height:Int {
        get {
            return Int (boundingBox.bottom) - Int (boundingBox.top)
        }
    }
}
