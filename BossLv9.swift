...
//  Magic Game
//
//  Created by Tran Quoc Tuan on 10/26/17.
//  Copyright © 2017 Tran Quoc Tuan. All rights reserved.
//

import SpriteKit

class BossLv9: SKSpriteNode, Boss {
    var textureAtlas:SKTextureAtlas = SKTextureAtlas(named:"BossLv9")
    ...
    func spawn(parentNode:SKNode, position: CGPoint, size: CGSize = CGSize(width: 368, height: 229), moveDuration: Double, moveToPoint: CGPoint, gestureList: String) {
        ...
        self.texture = textureAtlas.textureNamed("Idle_0.png")
    ...
    func moveToCenterOfScene() {
       // let moveToCenterWithAnimation = SKAction.group(<#T##actions: [SKAction]##[SKAction]#>)
    ...
    func createAnimation() {
        ...
        for index in 0...32 {
            attackSequence.append(textureAtlas.textureNamed("Attack_\(index).png"))
        }
        ...
        for index in 0...25 {
            damageSequence.append(textureAtlas.textureNamed("Hurt_\(index).png"))
        }
        damageAnimation = SKAction.animate(with: damageSequence, timePerFrame: 0.1, resize: false, restore: true)
        
        var standSequence: [SKTexture] = [SKTexture]()
        for index in 0...12 {
            standSequence.append(textureAtlas.textureNamed("Flight_\(index).png"))
        }
        ...
        for index in 0...12 {
            dieSequence.append(textureAtlas.textureNamed("Dead_\(index).png"))
        }
        dieAnimation = SKAction.animate(with: dieSequence, timePerFrame: 0.15, resize: true, restore: false)
        
        var flightSequence: [SKTexture] = [SKTexture]()
        for index in 0...12 {
            flightSequence.append(textureAtlas.textureNamed("Flight_\(index).png"))
        }
        flightAnimation = SKAction.animate(with: flightSequence, timePerFrame: 0.03, resize: true, restore: false)
